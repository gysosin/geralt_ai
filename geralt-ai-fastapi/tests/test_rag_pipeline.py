...
def close_async_clients_after_module():
    """Close async singleton clients opened by backwards-compatible shims."""
    yield
    try:
        from core.clients.elasticsearch_client import get_elasticsearch_client

        asyncio.run(get_elasticsearch_client().client.close())
    except Exception:
        pass
...
@pytest.mark.asyncio
async def test_elasticsearch_sample_documents(collection_id=None):
    ...
    try:
        ...
        result = await es_client.search(index=Config.ELASTIC_INDEX, body=query)
...
async def test_config_consistency():
    ...
    try:
        from clients import es_client
        mapping = await es_client.indices.get_mapping(index=settings.ELASTIC_INDEX)
...
async def run_all_tests():
    ...
    es_ok, es_count = await test_elasticsearch_connection()

    # Step 3: Sample ES documents
    if es_ok and es_count > 0:
        await test_elasticsearch_sample_documents(collection_id)
