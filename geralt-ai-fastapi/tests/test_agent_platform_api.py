...
def test_list_workflow_runs_endpoint_filters_by_status():
    run_db = MagicMock()
    run_db.find.return_value = []
    ...
    with patch("models.database.MongoClient"):
        with patch("core.clients.redis_client.redis.StrictRedis"):
            with patch("core.clients.minio_client.Minio"):
                ...
                client = TestClient(app)
                response = client.get(
                    "/api/v1/agent-platform/workflow-runs?status=blocked"
                )
                app.dependency_overrides.clear()
...
def test_list_workflow_runs_endpoint_passes_offset():
    run_db = MagicMock()
    cursor = MagicMock()
    cursor.sort.return_value = cursor
    ...
    with patch("models.database.MongoClient"):
        with patch("core.clients.redis_client.redis.StrictRedis"):
            with patch("core.clients.minio_client.Minio"):
                ...
                client = TestClient(app)
                response = client.get(
                    "/api/v1/agent-platform/workflow-runs?limit=25&offset=25"
                )
                app.dependency_overrides.clear()
...
def test_list_workflow_runs_endpoint_rejects_oversized_offset():
    run_db = MagicMock()
    ...
    with patch("models.database.MongoClient"):
        with patch("core.clients.redis_client.redis.StrictRedis"):
            with patch("core.clients.minio_client.Minio"):
                ...
                client = TestClient(app)
                response = client.get(
                    "/api/v1/agent-platform/workflow-runs?offset=10001"
                )
                app.dependency_overrides.clear()
